...
class RandomLoadText:
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        Randomly sample positive and negative texts and update class indices accordingly.
        """
        print("[RandomLoadText] 输入 labels keys:", list(labels.keys()))
        assert "texts" in labels, "No texts found in labels."
        class_texts = labels["texts"]
        print("[RandomLoadText] class_texts:", class_texts)
        num_classes = len(class_texts)
        print(f"[RandomLoadText] num_classes: {num_classes}")
        cls = np.asarray(labels.pop("cls"), dtype=int)
        print(f"[RandomLoadText] 原始 cls: {cls}")
        pos_labels = np.unique(cls).tolist()
        print("[RandomLoadText] pos_labels:", pos_labels)

        if len(pos_labels) > self.max_samples:
            print(f"[RandomLoadText] pos_labels 超过 max_samples({self.max_samples})，进行采样")
            pos_labels = random.sample(pos_labels, k=self.max_samples)
        print("[RandomLoadText] 采样后 pos_labels:", pos_labels)

        neg_samples = min(min(num_classes, self.max_samples) - len(pos_labels), random.randint(*self.neg_samples))
        print(f"[RandomLoadText] neg_samples: {neg_samples}")
        neg_labels = [i for i in range(num_classes) if i not in pos_labels]
        print("[RandomLoadText] neg_labels 候选:", neg_labels)
        if len(neg_labels) < neg_samples:
            print(f"[RandomLoadText] 警告: 可用负样本数 {len(neg_labels)} < 需要采样数 {neg_samples}")
        neg_labels = random.sample(neg_labels, k=neg_samples)
        print("[RandomLoadText] 最终采样 neg_labels:", neg_labels)

        sampled_labels = pos_labels + neg_labels
        print("[RandomLoadText] sampled_labels:", sampled_labels)
        # Randomness
        # random.shuffle(sampled_labels)

        label2ids = {label: i for i, label in enumerate(sampled_labels)}
        print("[RandomLoadText] label2ids:", label2ids)
        valid_idx = np.zeros(len(labels["instances"]), dtype=bool)
        new_cls = []
        for i, label in enumerate(cls.squeeze(-1).tolist()):
            if label not in label2ids:
                print(f"[RandomLoadText] 跳过 label {label}，不在 label2ids 里")
                continue
            valid_idx[i] = True
            new_cls.append([label2ids[label]])
        print(f"[RandomLoadText] new_cls: {new_cls}")
        labels["instances"] = labels["instances"][valid_idx]
        print(f"[RandomLoadText] 剩余有效 instances 数: {len(labels['instances'])}")
        ...
        for label in sampled_labels:
            ...
            print(f"[RandomLoadText] label {label} prompts: {prompts}")
            assert len(prompts) > 0
            prompt = self.prompt_format.format(prompts[random.randrange(len(prompts))])
            print(f"[RandomLoadText] 选中 prompt: {prompt}")
            texts.append(prompt)

        if self.padding:
            valid_labels = len(pos_labels) + len(neg_labels)
            num_padding = self.max_samples - valid_labels
            print(f"[RandomLoadText] padding: {self.padding}, num_padding: {num_padding}")
            if num_padding > 0:
                texts += random.choices(self.padding_value, k=num_padding)
                print(f"[RandomLoadText] padding_value: {self.padding_value}")

        print(f"[RandomLoadText] 最终 texts 长度: {len(texts)}，内容: {texts}")
        assert len(texts) == self.max_samples, f"texts 长度 {len(texts)} != max_samples {self.max_samples}"
        labels["texts"] = texts
        print("[RandomLoadText] 返回 labels keys:", list(labels.keys()))
        return labels
