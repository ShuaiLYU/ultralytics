...
def fuse_tp_vocab_into_pf_model(pf_model_weight,yaml_name,tp_model_weight,clip_weight_name="mobileclip2:b",open_ended_classes="./tools/open_ended_classes.txt"):
    ...
    one2one_vocab = tp_model.get_vocab(names)



    tp_model2=YOLOE(tp_model_weight)
    tp_model2.model.args['clip_weight_name']=clip_weight_name
    tp_model2.eval()
    tp_model2.cuda()
    tp_model2.args['clip_model_weight']=clip_weight_name
    with open('../buffer/ram_tag_list.txt', 'r') as f:
        names = [x.strip() for x in f.readlines()]
    tp_model2.model.end2end = False
    tp_model2.model.model[-1].end2end = False
    vocab = tp_model2.get_vocab(names)
    ...
    pf_model.set_vocab(vocab=None, names=names,one2one_vocab=one2one_vocab)

    pf_model.model.end2end = False
    pf_model.model.model[-1].end2end = False
    pf_model.set_vocab(vocab, names=names,one2one_vocab=None)
